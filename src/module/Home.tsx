      ...
      <section className="w-full">
        ...
        <div className="w-5/6 mx-auto flex flex-row justify-between my-5 items-start">
          <div className="grid grid-cols-3 grid-rows-7 w-4/6">
            ...
            {moreNews.map((more, i) => {
              return <div className={(i === 0) ? "relative col-span-2 w-full row-span-6 p-3 flex flex-col" : 'flex border border-thin border-red-300 flex-row justify-between items-center'}>
              ...
              <p className="text-sm px-1 py-1">{(i === 0) ? more.description : (more.title) ? `${more.title.substring(0, 44)} ...` : more.title}</p>
              </div>
            })}
          </div>
          <SideBar></SideBar>
