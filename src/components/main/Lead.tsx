...
const Lead: FunctionComponent<Props> = ({ page, data: topFive, isPending, error }: Props): ReactElement => {
  return (
    <div className="w-5/6 mx-auto">
      <section className="h-96 mx-auto gap-2 grid-rows-2 grid-cols-5 grid w-full">
        {topFive.map((tops, i) => {
          ...
          return (
            <div key={i} className={`relative ${classPlace}`}>
              <div className="absolute text-white flex flex-col justify-end p-6 z-20 h-full w-full bg-opacity-40 bg-black top-0 left-0">
                ...
                <h3 className="font-bold">
                  {tops.author ? tops.author : tops.source.name}
                </h3>
                <p className="text-sm">
                  {tops.description} <br />
                  {timeLaps.format(
                    -new Date(tops.publishedAt).getHours(),
                    "hours"
                  )}
                </p>
              </div>
